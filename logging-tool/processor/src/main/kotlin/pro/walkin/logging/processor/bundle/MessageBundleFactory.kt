...
internal class MessageBundleFactory(
    ...
    private fun messageMethods(): List<MessageMethod> {
        val messageMethods = listOf<MessageMethod>()

        val methodTranslations = methodTranslations()

        return definitionSource.messageFunctions.map { functionSource ->

            val translations = methodTranslations.getOrDefault(functionSource.name, mapOf())

            val parameters = functionSource.parameters().map { parameterSource ->
                val parameterType = parameterSource.type
                ...
                Parameter(
                    ...
                    objectType = parameterType.toString()
                )
            }

            val returnType = functionSource.returnType()
            val aa = returnType!!.resolve()
...
//            val javaThrowable = context.resolver.getClassDeclarationByName("java.lang.Exception")?.asType(emptyList())
            ...
            val isKotlinException = kotlinThrowable?.isAssignableFrom(aa)
//            val isJavaException = javaThrowable?.isAssignableFrom(aa)

            val isThrowable =  isKotlinException == true
...
//    }
     ...
     *
     */
    private fun methodTranslations(): Map<String, Map<String, String>> {
